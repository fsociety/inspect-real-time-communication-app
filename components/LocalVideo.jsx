...
export default function LocalVideo() {
    ...
    const { sfuVideoRoom } = contextData;
    ...
    useEffect(() => {
        if (!sfuVideoRoom) return;

        sfuVideoRoom.onlocalstream = stream => {
    ...
    function toggleMute() {
        ...
        if (muted) contextData.sfuVideoRoom.unmuteAudio();
        else contextData.sfuVideoRoom.muteAudio();
    ...
    function unpublishOwnFeed() {
        ...
        sfuVideoRoom.send({ message: unpublish });
    }
            ...
            {sfuVideoRoom ? (
                <div className='relative group'>
                    ...
                    <div className='absolute bottom-0 left-0 right-0 flex justify-between gap-1 px-4 py-8 transition-all duration-500 opacity-0 bg-gradient-to-t from-black/80 group-hover:opacity-100'>
                        <button onClick={toggleMute} className='px-3 py-1 text-sm rounded-sm btn-1'>
                            {isMuted ? 'unmute' : 'mute'}
                        </button>
                        <span className='font-bold' id='publisher'>
                            {sfuVideoRoom.publisher}
                        </span>
                        {sfuVideoRoom.message && <span className='message-box'>{sfuVideoRoom.message}</span>}
                        <Select
                            defaultValue={0}
                        ...
                        />
                        <button onClick={togglePublish} className='px-3 py-1 text-sm rounded-sm btn-1'>
                            Unpublish
                        </button>
                    </div>
                </div>
